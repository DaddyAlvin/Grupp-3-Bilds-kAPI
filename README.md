tisdag 08/09 Aleksander: struktur planering, gridlayout, skiss av sidan

Edwin: jag har lagt till bilder på hemsidan och sedan connectat alvins fina API till min API fetch!!!! så alvin och jag jobbar tillsammans dÅ!!!! yippie!!! Man kan också söka efter bilder!!! via tags!!!!

Alvin: första prototypen av API calling skript, samspel mellan Edwins javascript som behandlar datan, bugfixning och säkerhet emot för många requests

onsdag 09/09:

Alvin: Idag har jag bugfixat med apin i backenden, sen har jag mergat ihop projektet till main för nya branches och till sist har jag även påbörjat med inlogg och likesystem
###
Edwin: Gjort research om hur kan använder leaflet. Samt har jag och Alvin diskuterat lite hur vi kan gå vidare med vårat projekt!!. Samt har jag också fixat en bättre FYP (for you page) som tar data ifrån dina tidigare sökningar och använder den datan för att visa bilder på hemskärmen.
###
Aleksander: Jag ändrade om searchbaren så den ser bättre ut, jag anpassade bilderna(storlek, border radius mm), lade till hover effekt på bilderna.

Torsdag 10/09:

Avin: Idag har jag förbättrat prestandan på sidan och löst vissa buggar, sen har jag också byggt både ett inloggssystem samt likesystem med hjälp av en databas. Idag har jag även justerat lite filstruktur och filkopplingar.

###

Aleksander: justera färgschemat, anpassa undersidorna

###
Edwin:

fredag 11/09
Aleksander: gjorde css på login/sign up sidan, mobilanpassade startsidan, 
